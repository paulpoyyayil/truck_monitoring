...
def admin_add_driver(request):
    ...
    if request.method == 'POST':
        ...
        if (log.objects.filter(username=username)):
            ...
            l_id = login_data.id
            print(l_id)
            userdata = models.driver(name=name, email=email,mobile=mobile,location=location,vehicle_status=vehicle_status,licence_no=licence_no,transport_from=transport_from,transport_to=transport_to, login_id_id=l_id, status=driver_status, role=role)
            userdata.save()
            return redirect('view_approved_driver')
